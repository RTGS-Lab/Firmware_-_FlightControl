...
//BEGIN TESTS
//////////////////////////////////////////////////////////////////////////////
// verify critical fault is set rue if ioOB begin fails
TEST_F(KestrelTest, BeginIOBBeginFail) {
    ...
    EXPECT_CALL(mockIoTalon, begin())
        .WillOnce(::testing::Return(0));
    
    EXPECT_CALL(mockRtc, begin(true))
        .WillOnce(::testing::Return(true));
    
    EXPECT_CALL(mockGps, begin())
...
// verify critical fault is set if ioTalon begin fails
TEST_F(KestrelTest, BeginIOTalonBeginFail) {
    Kestrel kestrel = createFullyMockedKestrel();
    bool criticalFault = false;
    bool fault = false;
    float data[3] = {0.0, 0.0, 0.0};
...
// verify that led setOupput is only called when initDone is false
TEST_F(KestrelTest, BeginLEDsetOutputOnlyCalledWhenInitDoneFalse) {
Kestrel kestrel = createFullyMockedKestrel();
bool criticalFault = false;
bool fault = false;
float data[3] = {0.0, 0.0, 0.0};
...
result = kestrel.begin(0, criticalFault, fault);
...
//verify that critical fault is set if rtc.begin fails
TEST_F(KestrelTest, BeginRTCFail) {
    Kestrel kestrel = createFullyMockedKestrel();
    bool criticalFault = false;
    bool fault = false;
    float data[3] = {0.0, 0.0, 0.0};
...
//verify that critical fault is set if gps.begin fails
TEST_F(KestrelTest, BeginGPSFail) {
    Kestrel kestrel = createFullyMockedKestrel();
    bool criticalFault = false;
    bool fault = false;
    float data[3] = {0.0, 0.0, 0.0};
...
//verify that critical fault and fault are false when all begin calls are successul
TEST_F(KestrelTest, BeginAllSuccess) {
    Kestrel kestrel = createFullyMockedKestrel();
    bool criticalFault = false;
    bool fault = false;
    float data[3] = {0.0, 0.0, 0.0};
...
TEST_F(KestrelTest, EnablePowerPort5ReturnsImmediately) {
    ...
    EXPECT_CALL(mockIoTalon, pinMode(5, ::testing::_))
        .Times(0); // Should not be called for port 5
...
TEST_F(KestrelTest, EnablePowerInvalidPortThrowsError) {
    ...
    EXPECT_CALL(mockIoTalon, pinMode(0, ::testing::_))
        .Times(0); // Should not be called for invalid ports
...
TEST_F(KestrelTest, EnablePowerValidPortOperation) {
    ...
    EXPECT_CALL(mockIoTalon, pinMode(3, IIOExpander::IPinMode::OUTPUT)) // EN[0] for port 1
        .Times(1);
    EXPECT_CALL(mockIoTalon, digitalWrite(3, true))
        .Times(1);
...
// Test for WDT not being fed
TEST_F(KestrelTest, FeedWDTCriticalFault) {
    Kestrel kestrel = createFullyMockedKestrel();
    bool criticalFault = false;
    bool fault = false;
    float data[3] = {0.0, 0.0, 0.0};
    ...
    bool result = kestrel.feedWDT();
    EXPECT_FALSE(result); // Should return false if critical fault is set
}
...
TEST_F(KestrelTest, GetTimeValidTimeProvider) {
    ...
    EXPECT_CALL(mockTimeProvider, now())
        .WillRepeatedly(::testing::Return(1585699200)); // April 1, 2020
    
    time_t time = kestrel.getTime();
    EXPECT_EQ(time, 1585699200);
}

TEST_F(KestrelTest, GetTimeInvalidTimeProvider) {
    Kestrel kestrel = createFullyMockedKestrel();
...
// TEST_F(KestrelTest, TestTimeStringInvalidTime) {
//     Kestrel kestrel = createFullyMockedKestrel();
